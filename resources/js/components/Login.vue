<template>
  <div class="fixed flex-col flex justify-center items-center top-0 left-0 right-0 bottom-0 bg-gray-800 bg-opacity-90">
    ...
    <div @click.stop="cardClick" id="card" class="card bg-white w-11/12 p-4 rounded-md shadow-md">
      <div class="mb-4">
        <label for="email" class="sr-only">Email</label>
        <div class="bg-gray-100 w-full rounded-lg overflow-hidden">
          <input v-model="form.email" type="text" placeholder="Your email" id="email" autocomplete="off" name="email" class="bg-gray-100 border-2 w-full p-4 rounded-lg" >
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="js-field bg-gray-100 w-full rounded-lg overflow-hidden relative">
          <input v-model="form.password" type="password" placeholder="Choose a password" id="password" autocomplete="off" name="password" class="bg-gray-100 border-2 w-full py-4 pl-4 pr-10 rounded-lg " >
        </div>
      </div>
  
      <div class="mb-4 flex justify-between px-2">
        <div class="flex items-center">
          <input v-model="form.remember" type="checkbox" name="remember" id="remember" class="mr-2">
          <label for="remember">Remember me</label>
        </div>
        <a @click.prevent="switchToSignIn" href="#" class="hover:underline transition-all text-blue-400">Sign in</a>
      </div>
      ...
      <div>
        <button @click="send" type="submit" :class="{ 'disabled' : isDisabled}" class="bg-orange-600 hover:bg-orange-700 transition-all  text-white px-4 py-3 rounded w-full  font-medium ">Login</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props,context){
    ...
    // on envoie le formulaire au parent
    const send = () => {
      context.emit('login',form)
    }

    // permet de fermet la boite modal de connexion lorsqu'on clique a l'exterieure de la boite
    const closeModal = (e)=>{
      e.stopPropagation()
      context.emit('close')
    }
    // empêche la propagation du clique de la boite au parent pour ne pas executer la methode 'closeModal'
    const cardClick = (e) => {
      e.stopPropagation()
    }
    // permet de switcher vers le composant 'register' depuis le omposant login
    const switchToSignIn = () =>{
      context.emit('switch','register')
    }
    return {
      closeModal,
      cardClick,
